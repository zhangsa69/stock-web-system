...
> 基于巨潮资讯网财报与 NotebookLM 的 A 股 / 港股 AI 财报分析平台。
...
元基鉴股是一个面向 A 股和港股的财报分析 Web 平台。用户输入股票代码后，系统通过 Hermes Agent 调用 `cninfo-financial-analysis` 流程，获取巨潮资讯网的年报、半年报和季报，交由 NotebookLM 完成深度分析，并在平台内保存结构化报告。

当前项目的分析范围是 **A 股 / 港股**。代码中没有美股分析管道，也没有 UZI-Skill；前端目前明确提示美股分析仍在开发中。
...
- 美股分析目前尚未实现
- 没有 UZI-Skill 或 `deep-analysis` 分析管道
- 没有美国持仓监控功能
- 不包含自动邮件通知：当前分析任务中的邮件发送调用已禁用，报告以平台历史记录和下载为主
